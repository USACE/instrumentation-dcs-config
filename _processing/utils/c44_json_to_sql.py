...
def lookup_midas_param_info(configsensor_obj, midas_units, midas_params):
    ...
    # if raw and unknown unit, try last ditch effort
    if unit_id == '3254f483-5e66-405c-acf2-2a8add714bf5':
        if param_name.lower() == 'stage' or param_name.lower() == 'elevation':
...
instrument_sql = 'INSERT INTO midas.instrument(' \
...
instrument_status_sql = 'INSERT INTO midas.instrument_status(id, instrument_id, ' \
    'status_id, "time")\n VALUES \n'
...
for d in data:
    ...
    if name not in unique_sites:
        ...
        if config_sensors.keys():
            ...
            # Don't add these timeseries - they will cause duplicates
...
instrument_telemetry_sql = 'INSERT INTO midas.instrument_telemetry (instrument_id, ' \
...
if telemetry_goes:
    ...
    for tg in telemetry_goes:
        telemetry_goes_sql += f"INSERT INTO midas.telemetry_goes (id, nesdis_id) select '{tg[0]}', '{tg[1]}' where not exists (select 1 from midas.telemetry_goes where nesdis_id = '{tg[1]}');\n"
    # Replace the last line ending comma with semi-colon
...
if telemetry_iridium:
    ...
    for ti in telemetry_iridium:
        telemetry_iridium_sql += f"INSERT INTO midas.telemetry_iridium (id, imei) select '{ti[0]}', '{ti[1]}' where not exists (select 1 from midas.telemetry_iridium where imei = '{ti[1]}');\n"
    # Replace the last line ending comma with semi-colon
...
# Timeseries SQL Prep
timeseries_sql = 'INSERT INTO midas.timeseries(id, slug, name, instrument_id, ' \
